...
// end class node
    ...
    private int findMinHelper(Node node) {
        if (node == null) {
            return Integer.MAX_VALUE; // Base case: if the current node is null, return maximum integer value
        }
        ...
        // Return the minimum of the current node's data and the minimums from subtrees
...
// Recursively count nodes in the left and right subtrees
    ...
    public double average() {
        int[] count = new int[1]; // Array to hold count of nodes
        int[] sumAndCount = averageHelper(root); // Call the helper method to get the sum and count of nodes
        return sumAndCount[1] == 0 ? 0 : (double) sumAndCount[0] / sumAndCount[1];
        // If the count is 0, return 0 to avoid division by zero; otherwise, calculate the average
    }

    private int[] averageHelper(Node node) {
        // if the current node is null, return an array with sum 0 and count 0
        if (node == null) {
            return new int[] {0, 0};
        }
...
// Calculate total sum and total count from the current node and its subtrees
